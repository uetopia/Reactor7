...
void AMyProjectile::OnHit_Implementation(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComponent, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (TheCharacter) {
		...
		// It wasn't a character hit. 
		// Make sure it's not the control point actor either.

		AMyControlPointActor* TheCharacter = Cast<AMyControlPointActor>(OtherActor);
		if (TheCharacter) {
			UE_LOG(LogTemp, Log, TEXT("[UETOPIA] [AMyBomb] [OnHit] hit control point actor"));
		}
		else {
			//Just destroy it.
