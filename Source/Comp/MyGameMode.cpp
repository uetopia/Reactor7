...
{
	// check the tag and match it up with the team ID
	AMyPlayerState* ThisPlayerState = Cast<AMyPlayerState>(Player->PlayerState);

	for (TActorIterator<APlayerStart> It(GetWorld()); It; ++It)
	{
		APlayerStart* TestSpawn = *It;

		if (ThisPlayerState->TeamId == 1) {
			if (It->PlayerStartTag == "1") {
				return TestSpawn;
			}
		}
		else {
			if (It->PlayerStartTag == "2") {
				return TestSpawn;
			}
		}
	}
